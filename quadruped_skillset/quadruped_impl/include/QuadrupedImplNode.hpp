#ifndef QUADRUPED_SKILLSET_MANAGER_NODE_HPP
#define QUADRUPED_SKILLSET_MANAGER_NODE_HPP
...
{
public:
    QuadrupedImplNode() : SKILLSET_NODE("quadruped_example", "info") {
        current_location.target.data = "L0";
        this->set_data_location(current_location.target);
        this->declare_parameter("skill_execution_delay_sec", 0.0);
        this->declare_parameter("skill_failure_mode", "none");
        this->declare_parameter("skill_failure_target", "");
    }

    bool should_fail_skill(const std::string& skill_name);
    //-------------------- Event Hook --------------------
    ...
    quadruped_example_skillset_interfaces::msg::SkillGoToInput current_location;

    std::set<std::string> skills_already_failed_;
};
#endif
